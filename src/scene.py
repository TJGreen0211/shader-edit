...
class Scene(GlfwRenderer):
    ...
    def reload_shaders(self):
        shader = Shader(
            self.config_dict["shader_vs"], self.config_dict["shader_fs"])
        print(f"Shader Error: {shader.shader_error}")
    ...
    def draw_object(self):
        ...
        try:
            ...
            glUniformMatrix4fv(u_loc, 1, False, np.array(
                self.perspective).flatten().tobytes())
