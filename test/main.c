...
DLL_EXPORT void geometry_load_object_file(const char *fname, geometry *mesh)
{
	...
	//	//printf("%d\n", faces[i]);
	//	vec3Print(mesh->points[i]);
	//}

	//for(int i = 0; i < Index; i++) {
	//	printf("%f, %f, %f, ", mesh->points[i].v[0], mesh->points[i].v[1], mesh->points[i].v[2]);
	//}
	free(verts);
	free(faces);
...
DLL_EXPORT void geometry_load_object_file_depricated(const char *fname, geometry *mesh)
{	
	FILE *fp;
	fp = fopen(fname, "r");
...
//gcc -Wall -o main main.c matrixMath.c -lm
int main(int argc, char *argv[]) {
    geometry mesh;
    geometry_load_object_file("../resources/objects/cube.obj", &mesh);
    return 0;
}
